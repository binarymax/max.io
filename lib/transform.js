...
function guess_field(doc, possibles) {
    ...
    for (let i = 0; i < possibles.length; i++) {
        const p = possibles[i];
        if (p && p[0] && p[1]) {
            let found = doc.window.document.querySelector(p[0]);
            if (found instanceof Array) found = found[0]
            if (found && found[p[1]]) val = found[p[1]];
        } else if(p && p[0]) {
            let found = doc.window.document.querySelector(p[0]);
            if (found instanceof Array) found = found[0]
            if (found && found.innerHTML) val = found.innerHTML;
        }
    }
    return val;
}
...
function transform(url, html_string) {
    ...
    try {
        ...
        if(brightspot) {
            ...
            published = published||brightspot.publishedDate;
            author = author||brightspot.author;
        }
        /*
        <meta name="brightspot-dataLayer" content="{
