...
class Browser {
    ...
    download = async function(url,ignore) {

        const self = this;
        const cached = await self.cache.get(url);
        if (cached && !ignore) {
            console.log('cached!',self.name, '\t| ', url)
            return cached;
            ...
            if (browserSuccess) {
                const { error, data } = transform(url, browserResult.value.html);
                if(!error && data) await self.cache.set(url,data);
                return error ? null : data;
            } else if (fetchSuccess) {
                const { error, data } = transform(url, fetchResult.value.html);
                if(!error && data) await self.cache.set(url,data);
                return error ? null : data;
            }
